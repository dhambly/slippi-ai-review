...
def build_html(payload: dict[str, Any]) -> str:
    ...
    </style></head><body><header class="top"><strong>Nightly Melee Review</strong><a href="/">Dashboard</a></header><main><section class="hero"><span class="eyebrow">{_esc(session.get('date'))} session</span><h1>{_esc(session.get('headline') or 'The habits worth practicing tomorrow')}</h1><p>{recurring_evidence} of {int(payload.get('evidenceCount') or 0)} qualified comparisons repeated across games. Treat these as cross-game hypotheses from four-sample rollouts and verify each linked position before drilling it.</p><div class="stats"><div><strong>{int(stats.get('analyzedGames') or 0)}</strong><span>games analyzed</span></div><div><strong>{int(payload.get('evidenceCount') or 0)}</strong><span>qualified comparisons</span></div><div><strong>{len(patterns)}</strong><span>repeated patterns</span></div><div><strong>{_esc(stats.get('processingTime') or 'n/a')}</strong><span>processing time</span></div></div></section><div class="section-head"><div><span class="eyebrow">Start here</span><h2>Practice priorities</h2></div><p>Ordered by recurrence, sample support, and outcome gap.</p></div>{cards}{more}<div class="section-head"><div><span class="eyebrow">Evidence</span><h2>Every processed game</h2></div><p>Open a full phase deck for context.</p></div><div class="games">{game_links}</div></main><script>
