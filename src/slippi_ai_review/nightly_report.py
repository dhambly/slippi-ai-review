...
def _move_label(value: Any, character: str = "") -> str:
    ...
    if move == "down b" and character in {"Fox", "Falco"}:
        return "shine"
...
def _phillip_action(target: dict[str, Any], character: str) -> str:
...
def _last_commitment(segment: dict[str, Any], player_index: int, character: str) -> tuple[str, str]:
    ...
    keywords = (
        ...
        "side b", "up b", "neutral b", "landing", "fall", "turn", "wait", "crouch",
    )
    ...
    if "landing" in action:
        ...
        action = "jump"
    elif "dash" in action or "run" in action or action in {"turn", "wait"}:
...
def _option_rows(rows: Iterable[dict[str, Any]], frame: int, signature: str) -> list[dict[str, Any]]:
    return [
        row for row in rows
        if int(row.get("baseFrame") or -1) == frame and str(row.get("optionSignature") or "") == signature
    ]
...
def collect_review_evidence(review_dir: Path) -> list[tuple[str, Evidence]]:
    ...
    for target in queue.get("targets") or []:
        ...
        if samples < minimum_samples or share < 0.15 or reversal_rate > 0.35 or self_death_rate > 0.20:
            continue
        ...
        phillip_action = _phillip_action(target, character)
...
def _pattern_copy(phase: str, action: str, items: list[Evidence]) -> tuple[str, str, str]:
    ...
    if phase == "neutral":
        ...
        }.get(action, f"Getting opened during {action}")
        summary = (
            f"This showed up {len(items)} times during {replay_example}: the replay most often got caught by {opponent}, "
            f"while Phillip's stable branches most often began with {phillip}."
        )
        drill = (
            f"Open each linked position and use {phillip} at the point where the replay committed to {replay_example}. "
            "Stop at the first clean hit and complete 10 correct responses on each side."
        )
    elif phase == "advantage":
        title = f"Leaving damage after {action} openings"
        summary = (
            f"Across {len(items)} conversions, the replay's {action} opening ended materially earlier than stable Phillip branches, "
            f"which most often continued with {phillip}."
        )
        drill = (
            f"Start from the {action} opening and route into {phillip}; continue until neutral or the stock ends. "
...
def _evidence_card(item: dict[str, Any]) -> str:
    ...
    <article class="evidence" id="scenario-{_esc(item['review_id'])}-{int(item['target_index'])}">
      ...
      <div class="evidence-copy">
        ...
        <p><b>Replay:</b> {_esc(item['original_result'])} after {_esc(item['replay_action'])}.</p>
        <p><b>Phillip:</b> {_esc(item['phillip_result'])}.</p>
        <p class="support">This response appeared in {int(item['option_samples'])} of {int(item['sweep_samples'])} simulations.</p>
...
def _pattern_card(pattern: dict[str, Any], index: int) -> str:
    evidence = "".join(_evidence_card(item) for item in (pattern.get("evidence") or [])[:3])
    return f"""
